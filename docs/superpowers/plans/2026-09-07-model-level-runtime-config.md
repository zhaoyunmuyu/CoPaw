...
- [ ] Write failing tests for validation, update, delete cleanup, legacy `generate_kwargs` omission, and distribution payloads.
- [ ] Add typed per-model configuration and targeted read/write operations.
- [ ] Run focused pytest tests and commit.
...
- [ ] Write failing tests proving generation fields and provider-specific output limit mapping reach every factory-created model, and input budget overrides Agent defaults.
- [ ] Resolve model config in the factory; apply it before wrappers and preserve it in frozen Provider snapshots.
- [ ] Run focused pytest tests and commit.
...
- [ ] Write failing component/API tests for model config controls and removal of `generate_kwargs` editor.
- [ ] Add model management configuration dialog and current-model Chat thinking controls with immediate persistence.
- [ ] Run focused Vitest tests/build and commit.

### Task 4: Cross-cutting verification and review

**Files:** changed files and `CONTEXT.md`

- [ ] Run relevant Python and Console suites, formatting/linting, and graph change analysis.
- [ ] Request subagent specification and quality reviews; fix all findings.
