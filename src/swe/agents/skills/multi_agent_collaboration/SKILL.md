...
description: Use this skill when another agent's expertise/context is needed, or when the user explicitly asks to involve another agent. First list agents, then use swe agents chat for two-way communication with replies. | 当需要其他 agent 的专长/上下文，或用户明确要求调用其他 agent 时使用；先查 agent，再用 swe agents chat 双向通信（有回复）
metadata: { "builtin_skill_version": "1.1", "swe": { "emoji": "🤝" } }
---
...
| 任务类型 | 使用模式 | 命令 |
|---------|---------|------|
| 简单快速查询 | 实时模式 | `swe agents chat` |
| 复杂任务（数据分析、批量处理等） | 后台模式 | `swe agents chat --background` |
...
2. swe agents list
3. swe agents chat --background 提交任务
4. 从输出中记录 [TASK_ID: ...]
5. 继续处理其他工作
...
Check status with:
  swe agents chat --background --task-id 20802ea3-...
```
...
# 方式 2：如果必须等待，使用合理间隔
sleep 30 && swe agents chat --background \
  --task-id 20802ea3-832d-4fb4-86f0-666ad79fcc80
```
...
# 3. 在适当时机查询结果
# （比如处理完当前任务后，或用户询问进度时）
swe agents chat --background --task-id <id>
```

**方式 2：定时轮询**（如果必须等待）
```bash
# 递增间隔，先快后慢
sleep 10 && swe agents chat --background --task-id <id>
sleep 20 && swe agents chat --background --task-id <id>
sleep 30 && swe agents chat --background --task-id <id>
```
