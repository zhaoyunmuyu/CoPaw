# -*- coding: utf-8 -*-
"""Token counting utilities for SWE using HuggingFace tokenizers.
...
class SweTokenCounter(HuggingFaceTokenCounter):
    """Token counter for SWE with configurable tokenizer support.
...
class SweEstimateTokenCounter(HuggingFaceTokenCounter):
...
_token_counter_cache: dict[
    tuple,
    SweTokenCounter | SweEstimateTokenCounter,
] = {}


def get_swe_token_counter(
    ...
    Args:
        ...
        use_estimate: If True (default), returns a SweEstimateTokenCounter
            that uses only character-based estimation without loading a
            tokenizer. If False, returns a full SweTokenCounter backed
    ...
    if use_estimate:
        return SweEstimateTokenCounter(
        ...
        if config_key not in _token_counter_cache:
            _token_counter_cache[config_key] = SweTokenCounter(
