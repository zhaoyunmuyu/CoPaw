...
def _copy_default_working_files(
    ...
    This copies all files and directories from the default user's working directory,
    including config.json, md files, active_skills, etc.
    ...
    Returns:
        List of copied file/directory names.
    """
    copied_files: list[str] = []
    exclude_set = set(exclude_files) if exclude_files else set()

    # Copy all files and directories from default user
    for item in src_working_dir.iterdir():
        if item.name in exclude_set:
            continue
        dst_item = dst_working_dir / item.name
        if dst_item.exists():
            continue
        try:
            if item.is_file():
            ...
            logger.debug("Copied from default user: %s", item.name)
        except Exception as e:
...
def _init_default_user(
    ...
    """Initialize default user from templates.

    Returns:
        Tuple of (config_copied, md_copied).
    """
    ...
    return config_copied, md_copied


def _init_from_default_user(
    user_id: str,
    working_dir: Path,
    secret_dir: Path,
    ...
    """Initialize non-default user by copying from default user.

    Returns:
        Tuple of (config_copied, providers_copied, md_copied).
    """
    config_copied = False
    ...
    return config_copied, providers_copied, md_copied
...
def _fallback_to_templates(
    ...
    language: str,
    config_copied: bool,
    providers_copied: bool,
    md_copied: bool,
) -> tuple[bool, bool, bool]:
    """Fallback to templates if copying from default user failed.

    Returns:
        Tuple of (config_copied, providers_copied, md_copied) - updated values.
    """
...
def _create_default_files(
    ...
    """Create default config.json, providers.json, and HEARTBEAT.md if missing."""
...
def initialize_user_directory(
    ...
    if user_id == "default":
        _init_default_user(user_id, working_dir, language)
    else:
    ...
    _create_default_files(user_id, config_path, secret_dir, language)
