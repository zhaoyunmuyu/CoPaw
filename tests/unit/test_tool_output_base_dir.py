...
from swe.security.tenant_path_boundary import AbsolutePathDeniedError, PathTraversalError
...
def test_browser_control_output_path_fails_closed_on_workspace_boundary_violation(
    ...
    with patch("swe.security.tenant_path_boundary.WORKING_DIR", tmp_path):
        with tenant_context(tenant_id="tenant_a", workspace_dir=outside_workspace):
...
@pytest.mark.asyncio
async def test_desktop_screenshot_default_path_falls_back_to_global_working_dir_without_tenant_context(
    tmp_path: Path,
):
    with patch("swe.agents.tools.desktop_screenshot._capture_mss") as capture:
        capture.return_value = _tool_ok(
