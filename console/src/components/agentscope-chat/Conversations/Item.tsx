...
const ConversationsItem: React.FC<ConversationsItemProps> = React.memo(
  (props) => {
    ...
    return (
      ...
      >
        {inViewport && info.icon && (
          <div className={`${prefixCls}-icon`}>{info.icon}</div>
        )}
        {inViewport && (
          <div className={`${prefixCls}-content-wrapper`}>
            <div className={`${prefixCls}-content`}>
              {(info.timeline || selectable) && (
                <div className={`${prefixCls}-timeline`}>
                  {selectable ? (
                    <div
                      className={`${prefixCls}-timeline-checkbox`}
                      onClick={(e) => e.stopPropagation()}
                    >
                      <Checkbox
                        checked={selected}
                        onChange={() => onSelect?.(info.key, !selected)}
                      />
                    </div>
                  ) : (
              ...
              {menu && !disabled && !selectable && (
                <Popover
                  ...
                  content={
                    <div className={`${prefixCls}-menu-popover`}>
                      {menu.map((item) => {
                        const { key, ...rest } = item;
                        const _props = {
                          ...rest,
            ...
            {info.desc && (
              <div
                ...
                style={info.timeline || selectable ? { marginLeft: 16 } : {}}
              >
                {info.desc}
              </div>
            )}
...
function MenuItem(props) {
  ...
  if (label)
    return (
      <Button disabled={disabled} danger={danger} type="text" onClick={onClick}>
        {label}
      </Button>
    );
  return null;
