    ...
    >
      <Image.PreviewGroup>
        {images.map((image, index) => (
          <Image src={image} key={index} width={44} height={44} />
        ))}
      </Image.PreviewGroup>
    </ConfigProvider>
  );
...
function Item({ item }) {
  ...
  return (
    <div className={`${prefixCls}-rag-item`}>
      <div
        className={`${prefixCls}-rag-item-title`}
        onClick={() => {
          setOpen(!open);
        }}
      >
        ...
        {item.score ? (
          <Tag
            color="mauve"
            size="small"
            className={`${prefixCls}-rag-item-score`}
          >
            得分 <b>{item.score}</b>
          </Tag>
        ) : null}
      ...
      {open && (
        <div className={`${prefixCls}-rag-item-content`}>
          <div className={`${prefixCls}-rag-item-content-text`}>
            {item.content}
          </div>
          ...
          {item.link ? (
            <a
              ...
              className={`${prefixCls}-rag-item-footer`}
              href={item.link}
              target="_blank"
            >
              {item.footer}
...
export default function (props: IRagProps) {
    ...
    <OperateCard.LineBody>
      ...
      {images?.length ? (
        <div>
          ...
          <div
            className={`${prefixCls}-rag-group-content ${prefixCls}-rag-group-content-images`}
          >
      ...
      {props.list.length ? (
        ...
        <>
          ...
          <Flex
            ...
            justify="center"
            gap={8}
            className={`${prefixCls}-rag-empty-placeholder`}
          >
            <SparkWarningCircleFill />
  ...
  return (
    <OperateCard
      ...
      body={{
        defaultOpen,
        children: <div className={`${prefixCls}-rag-children`}>{children}</div>,
      }}
    />
  );
}
