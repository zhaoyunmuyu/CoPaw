  ...
  return (
    <>
      ...
      <div className={prefixCls} style={props.style}>
        <div
          className={`${prefixCls}-options ${value ? `${prefixCls}-hide` : ""}`}
        >
        ...
        <div
          className={`${prefixCls}-display ${value ? "" : `${prefixCls}-hide`}`}
        >
          <div className={`${prefixCls}-display-flex`}>
            <Display
              {...valueObject}
              label={valueObject?.selectedLabel || valueObject?.label}
              className={`${prefixCls}-display-label`}
            />
            {props.desc && (
              <div className={`${prefixCls}-display-desc`}>{props.desc}</div>
            )}
            {props.closeTip ? (
              <Tooltip title={props.closeTip}>{close}</Tooltip>
            ) : (
              close
            )}
          </div>
