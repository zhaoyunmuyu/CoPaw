...
function Item(props: IAccordionProps) {
  ...
  const icon = useMemo(() => {
    ...
    if (props.status === "generating")
      return <SparkLoadingLine className={`${prefixCls}-icon-loading`} spin />;
    if (props.status === "finished")
      return <SparkCheckCircleFill className={`${prefixCls}-icon-success`} />;
    if (props.status === "interrupted") return <SparkStopCircleLine />;
    if (props.status === "error")
      return <SparkErrorCircleFill className={`${prefixCls}-icon-error`} />;
  ...
  return (
    <div className={cls(`${prefixCls}`, `${prefixCls}-${status}`)}>
      <div
        className={cls(`${prefixCls}-header`, `${prefixCls}-header-${status}`)}
        onClick={() =>
          content && props.open === undefined && setStateOpen(!stateOpen)
        }
      >
        {icon ? (
          <div
            className={cls(`${prefixCls}-header-icon`, {
              [`${prefixCls}-header-icon-line`]: props.iconLine,
              // @ts-ignore
              [`${prefixCls}-header-icon-first`]: props.isFirst,
              // @ts-ignore
              [`${prefixCls}-header-icon-last`]:
                (props.isLast && status === "close") || props.level,
            })}
          >
            {icon}
          </div>
        ...
        <div>{props.title}</div>
        {content && (
          <div className={cls(`${prefixCls}-header-arrow`)}>
...
function Children(props: any) {
  ...
  return (
    <Transition nodeRef={nodeRef} in={stateOpen} timeout={300}>
      {(state) => (
        <div
          style={{
            ...bodyStyle,
          ...
          className={cls(
            `${prefixCls}-body`,
            `${prefixCls}-body-${stateOpen ? "open" : "close"}`,
            {
